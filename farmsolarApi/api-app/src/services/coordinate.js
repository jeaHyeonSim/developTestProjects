...
class CoordinateService {
    async getNsdi_landCharacteristicsWfs(dto) {
		const convert = require('xml-js');
        let authkey = properties.nsdi_landCharacteristicsWfsKey;
        return new Promise(function (resolve, reject) {
            ...
            parameter += "&" + encodeURIComponent("pnu") + "=" + encodeURIComponent(dto.pnu); /* 필지고유번호 19자리중 최소 8자리(시도[2]+시군구[3]+읍면동[3])(입력시 bbox값은 무시) */  
    ...
    // kakao 지도 좌표값 얻기
    async getKakao_kakaoAddress(dto) {
        try {
            let kakaoAKkey = properties.kakaoAKkey;
            let url = encodeURI(`https://dapi.kakao.com/v2/local/search/address.json?query=${dto.jibunAddr}`);
