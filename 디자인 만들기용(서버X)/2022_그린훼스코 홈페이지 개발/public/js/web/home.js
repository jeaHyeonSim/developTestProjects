...
$(function(){
	...
	$('#header .menuBox > ul.mainManu > li').on({
		mouseenter  : function(){
			let liFirstHeight = Number($(this).find('.subMenu> li').eq(0).css('height').split("p")[0]);
			let liHeight = Number($(this).find('.subMenu> li').eq(1).css('height').split("p")[0]);
			let liMT = Number($(this).find('.subMenu> li').eq(1).css('margin-top').split("p")[0]);
			let liLen = $(this).find('.subMenu> li').length - 1;
			$(this).addClass("on");
			$(this).find('.subMenu').css({
				'height' : `${liFirstHeight + (liLen * (liHeight + liMT))}px`,
				'padding': '15px 0'
			});
		},
		mouseleave : function(){
	...
	$('#header .menuBox > ul.mainManu > li > ul.subMenu').on({
		mouseover : function(){
			// $(this).children('.subMenu').slideDown(400).show();
			// $(this).siblings('.subMenu').slideDown(400).css({
			// 	'display' : 'block'
			// });
			// $(this).css({
			// 	'display' : 'block'
		...
		mouseout : function(){
			...
			// $(this).siblings('.subMenu').slideUp(200).css({
			// 	'display' : 'block'
			// });;
	...
	// 사이즈 아이콘 ()
	$('#header .listMenu .icon_menu').on({
		mouseover : function(){
			$(this).children('span').eq(1).css({
				'margin-left': '0px',
				'animation-name': 'sideLineIn'
			});
		},
		mouseout : function(){
			$(this).children('span').eq(1).css({
				'margin-left': '5px',
				'animation-name': 'sideLineOut'
			});
		}
	});
	$('#header .listMenu .icon_menu').on('cilck', function() {
	...
	*/
	...
	// 한글 영문 클릭 이벤트
	let krEnBtn = document.querySelectorAll('#header > div.headerBtn > div.KrEn span');
	krEnBtn.forEach(element => {
